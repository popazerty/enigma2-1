#
#  AddOnOpendroid for OE 2.0
#  Coded Opendroid
#  Support: www.droidsat.org
#
...
def main(session,**kwargs):
    session.open(OpendroidDownloadBrowser)

def Plugins(**kwargs):
    return [PluginDescriptor(name="Addon Opendroid", description=_("Manage your additional software"), where = [PluginDescriptor.WHERE_PLUGINMENU, PluginDescriptor.WHERE_EXTENSIONSMENU ], fnc=main, icon = "plugin.png")]

class OpendroidPluginList(MenuList):
...
class OpendroidDownloadBrowser(Screen):

	skin = """
		<screen name="OpendroidDownloadBrowser" position="center,center" size="660,475" title="Addon Opendroid">
	...
	def __init__(self, session, args = None):
		...
		self.list = []
		self["list"] = OpendroidPluginList(self.list)
		self.pluginlist = []
		self.expanded = []
		self.addoninstalled = []
		self.found = 0
	...
	def setWindowTitle(self):
		self.setTitle(_("Opendroid AddOn Browser"))

	def startRun(self):
		print "startRun(self):"
		self["list"].instance.hide()
		if fileExists("/usr/lib/enigma2/python/Plugins/Extensions/AddOnOpendroid/debug"):
			print "[AddonOpendroid] running opkg update"
	...
	def installFinished(self):
		...
		self["list"].instance.hide()
		try:
			f = open("/var/lib/opkg/info/"+self["list"].l.getCurrentSelection()[0].name+".control", "r")
			addoncontent = f.read()
			f.close()
		except:
			addoncontent = ""
	...
	def runFinished(self, retval):
		if fileExists("/usr/lib/enigma2/python/Plugins/Extensions/AddOnOpendroid/debug"):
			print "[AddonOpendroid] finished command execution. Return value is: ", retval
		self.remainingdata = ""
		if self.run == 0:
			self.run = 1
			if fileExists("/usr/lib/enigma2/python/Plugins/Extensions/AddOnOpendroid/debug"):
				print "[AddonOpendroid] running opkg list-installed enigma2-*"
			self.container.execute("opkg list-installed enigma2-*")
		elif self.run == 1:
			self.run = 2
			if fileExists("/usr/lib/enigma2/python/Plugins/Extensions/AddOnOpendroid/debug"):
				print "[AddonOpendroid] running opkg list enigma2-*"
			self.container.execute("opkg list enigma2-*")
		elif self.run == 2:
			if fileExists("/usr/lib/enigma2/python/Plugins/Extensions/AddOnOpendroid/debug"):
				print "[AddonOpendroid] run is 2"
	...
	def dataAvail(self, str):
		...
		if fileExists("/usr/lib/enigma2/python/Plugins/Extensions/AddOnOpendroid/debug"):
			print "[AddonOpendroid] got data", str
		...
		for x in lines:
			if fileExists("/usr/lib/enigma2/python/Plugins/Extensions/AddOnOpendroid/debug"):
				print "[AddonOpendroid] processing data. Current line: ", x
			plugin = x.split(" - ")
			if len(plugin) >= 2 and self.run == 1:
				if fileExists("/usr/lib/enigma2/python/Plugins/Extensions/AddOnOpendroid/debug"):
					print "[AddonOpendroid] length of plugin is >=2"
				self.addoninstalled.append(AddOn(name = plugin[0], version = plugin[1], status = 1))
			elif len(plugin) == 3 and self.run == 2:
				if fileExists("/usr/lib/enigma2/python/Plugins/Extensions/AddOnOpendroid/debug"):
					print "[AddonOpendroid] length of plugin is 3"
	...
	def updateList(self):
		if fileExists("/usr/lib/enigma2/python/Plugins/Extensions/AddOnOpendroid/debug"):
			print "[AddonOpendroid] updating list"
		self.list = []
		...
		installedIcon = LoadPixmap(resolveFilename(SCOPE_PLUGINS, "/usr/share/enigma2/Opendroid/green.png"))
		notinstalledIcon = LoadPixmap(resolveFilename(SCOPE_PLUGINS, "/usr/share/enigma2/Opendroid/red.png"))
		updateIcon = LoadPixmap(resolveFilename(SCOPE_PLUGINS, "/usr/share/enigma2/Opendroid/blue.png"))
		self.plugins = {}
		for x in self.pluginlist:
			if fileExists("/usr/lib/enigma2/python/Plugins/Extensions/AddOnOpendroid/debug"):
				print "[AddonOpendroid] processing pluginlist", x
			temp = ""
			temp1 = x.name
			if x.name.startswith('enigma2-skin-') or x.name.startswith('enigma2-cams-') or x.name.startswith('enigma2-configs-') or x.name.startswith('enigma2-picons-') or x.name.startswith('enigma2-languages-') or x.name.startswith('enigma2-Opendroidepgcenter-skin-') or x.name.startswith('enigma2-piconmapper-'):
				temp = x.name[8:]
...
class DialogUpdateDelete(Screen):
	...
	skin = """
		<screen name="DialogUpdateDelete" position="60,245" size="600,10" title="AddOnOpendroid">
